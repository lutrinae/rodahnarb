...
public class Robot {
	...
	public Robot(World world, OrthographicCamera camera) {
		...
		filter = new Filter();
		filter.categoryBits = Platformer.CATEGORY_PLAYER;
		filter.maskBits = Platformer.MASK_PLAYER;
	...
	public void fire() {
		...
		if (ammo > 0 && (lastFired + reloadTime < System.currentTimeMillis())) {
			...
			float xDiff = Gdx.input.getX() * Platformer.WORLD_TO_BOX - bulletFixture.getBody().getPosition().x;
			float yDiff = Gdx.input.getY() * Platformer.WORLD_TO_BOX - bulletFixture.getBody().getPosition().y;
